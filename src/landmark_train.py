...
def get_one_batch(batch_size, type):
    if type == 0:  ##train
        file_list = tf.gfile.Glob("train.record")
    else:
        file_list = tf.gfile.Glob("test.record")
...
with tf.Session() as session:
    ...
    for step in range(100000):
        ...
        global_step_val, lr_val, _, loss_val, l2_loss_val = \
            session.run([global_step, lr, train_op, loss, l2_loss],feed_dict={input_x: batch_x, label: batch_y})
